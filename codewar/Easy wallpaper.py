...
def wallpaper(l, w, h):
    if l == 0 or w == 0 or h ==0:
        return numbers[0]
    square_room = 2 * l * h + 2 * w * h
    weidth_paper = 0.52
    length = 10
    square_paper = weidth_paper * h
    paper_piece = square_room / square_paper
    paper_need = (paper_piece * h * 1.15) / length
    need = math.ceil(paper_need)
    result = numbers[need]
    return result

print(wallpaper(5, 6.7, 2.81))


def wallpaper1(l, w, h):
    result = {
